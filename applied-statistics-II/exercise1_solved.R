...
propvalue<-predict(object = resultrob,
                   newdata=data.frame(age=5, parea=80, bath=1, ensuit=1,
...
### Valor adicional pelo imóvel ser apartamento (categ=1)###

# Se quisermos ver o efeito (valor adicional médio) de o imóvel
# ser apartamento (categ=1) então devemos reestimar o modelo 
# sem intercepto, pegar o parâmetro "categ" estimado no
# modelo robusto sem intercepto extrair o antilog dele e depois
# reverter o lambda do box-cox
# Se for um modelo sem box-cox, obviamente não é necessária a
# etapa de reverter o lambda

resultrob <- lmRob(lnpricetrans~age+parea+bath+
                     garag+park+balc+elev+fitg+party+categ-1, 
                   data = imoveiscwbav)
summary(resultrob)

antilogcateg <- exp(resultrob[["coefficients"]][["categ"]])
lambda <- outlm[["lambda.hat"]]
valorcateg1<-((antilogcateg*lambda)+1)^(1/lambda)
valorcateg1
