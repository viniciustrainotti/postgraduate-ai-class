...
# setwd("/opt/pos/r-ai-class/machine-learning/term-paper/pratica-imposto-de-renda-20220820")
dados <- read.csv("Material 03 - 7 – C - IR - Dados.csv")
View(dados)
...
set.seed(0758662190)
rna <- train(form= sonegador~., data=treino, method="nnet", tuneGrid=grid, trControl=ctrl, maxit=2000, trace=FALSE)
rna
predict.rna <- predict(rna, teste)
confusionMatrix(predict.rna, as.factor(teste$sonegador))
...
set.seed(0758662190)
rf <- train(sonegador ~ ., data=treino, method="rf", trace=FALSE)
rf
...
ctrl <- trainControl(method = "cv", number = 10)
set.seed(0758662190)
rf <- train(sonegador ~ ., data=treino, method="rf", trace=FALSE, trControl=ctrl)
rf
predict.rf <- predict(rf, teste)
confusionMatrix(predict.rf, as.factor(teste$sonegador))
...
set.seed(0758662190)
rf <- train(sonegador ~ ., data=treino, method="rf", trace=FALSE, trControl=ctrl, tuneGrid=tuneGrid)
rf
predict.rf <- predict(rf, teste)
confusionMatrix(predict.rf, as.factor(teste$sonegador))

## RF - Melhor Modelo - Predição novos casos

dados_novos_casos <- read.csv("Material 03 - 7 – C - IR - Dados - Novos Casos.csv")
...
View(resultado)
write.csv(resultado, "resultado-imposto-de-renda.csv")

## Salvar Modelo

getwd()
saveRDS(rf, "Material 03 - 7 – C - IR - Dados - Melhor Modelo.rds")

modelo_lido <- readRDS("Material 03 - 7 – C - IR - Dados - Melhor Modelo.rds")
