...
# setwd("/opt/pos/r-ai-class/machine-learning/term-paper/pratica-previsa-do-tempo-20220820")
dados <- read.csv("Material 03 - 6 - C - Previsao do Tempo - Dados.csv")
View(dados)
...
set.seed(0758662190)
rna <- train(form= Chovera~., data=treino, method="nnet", tuneGrid=grid, trControl=ctrl, maxit=2000, trace=FALSE)
rna
predict.rna <- predict(rna, teste)
confusionMatrix(predict.rna, as.factor(teste$Chovera))
...
set.seed(0758662190)
rf <- train(Chovera ~ ., data=treino, method="rf", trace=FALSE)
rf
...
ctrl <- trainControl(method = "cv", number = 10)
set.seed(0758662190)
rf <- train(Chovera ~ ., data=treino, method="rf", trace=FALSE, trControl=ctrl)
rf
predict.rf <- predict(rf, teste)
confusionMatrix(predict.rf, as.factor(teste$Chovera))
...
set.seed(0758662190)
rf <- train(Chovera ~ ., data=treino, method="rf", trace=FALSE, trControl=ctrl, tuneGrid=tuneGrid)
rf
predict.rf <- predict(rf, teste)
confusionMatrix(predict.rf, as.factor(teste$Chovera))
...
dados_novos_casos <- read.csv("Material 03 - 6 - C - Previsao do Tempo - Dados - Novos Casos.csv")
...
View(resultado)
write.csv(resultado, "resultado-previsao-do-tempo.csv")

## Salvar Modelo

getwd()
saveRDS(rf, "Material 03 - 6 - C - Previsao do Tempo - Dados - Melhor Modelo.rds")

modelo_lido <- readRDS("Material 03 - 6 - C - Previsao do Tempo - Dados - Melhor Modelo.rds")
novas_predicoes <- predict(modelo_lido, teste)
confusionMatrix(novas_predicoes, as.factor(teste$Chovera))
