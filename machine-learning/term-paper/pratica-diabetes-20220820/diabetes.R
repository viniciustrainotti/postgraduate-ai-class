...
# setwd("/opt/pos/r-ai-class/machine-learning/term-paper/pratica-diabetes-20220820")
dados <- read.csv("Material 03 - 9 – C - Diabetes - Dados.csv")
View(dados)
...
set.seed(0758662190)
rna <- train(form= diabetes~., data=treino, method="nnet", tuneGrid=grid, trControl=ctrl, maxit=2000, trace=FALSE)
rna
predict.rna <- predict(rna, teste)
confusionMatrix(predict.rna, as.factor(teste$diabetes))
...
set.seed(0758662190)
rf <- train(diabetes ~ ., data=treino, method="rf", trace=FALSE)
rf
...
ctrl <- trainControl(method = "cv", number = 10)
set.seed(0758662190)
rf <- train(diabetes ~ ., data=treino, method="rf", trace=FALSE, trControl=ctrl)
rf
predict.rf <- predict(rf, teste)
confusionMatrix(predict.rf, as.factor(teste$diabetes))
...
set.seed(0758662190)
rf <- train(diabetes ~ ., data=treino, method="rf", trace=FALSE, trControl=ctrl, tuneGrid=tuneGrid)
rf
predict.rf <- predict(rf, teste)
confusionMatrix(predict.rf, as.factor(teste$diabetes))
...
dados_novos_casos <- read.csv("Material 03 - 9 – C - Diabetes - Dados - Novos Casos.csv")
...
View(resultado)
write.csv(resultado, "resultado-diabetes.csv")

## Salvar Modelo

getwd()
saveRDS(rf, "Material 03 - 9 – C - Diabetes - Dados - Melhor Modelo.rds")

modelo_lido <- readRDS("Material 03 - 9 – C - Diabetes - Dados - Melhor Modelo.rds")
